...
func OptionalAuth(logger *logrus.Logger, authCfg *config.AuthConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		requestID := c.GetString(RequestIDKey)
		...
		authHeader := c.GetHeader("Authorization")
		if authHeader == "" {
			entry.Info("No Authorization header provided, proceeding as unauthenticated")
			c.Next()
			return
		}
